 ...
 * Mail
 *
 * based on mail.inc.php,v 1.6 2005/12/01 06:48:14 
 *
 * @package BitWiki
 * @author  youka
...
class Mail implements MyObserver
{
    ...
    public function getsending() { return (bool)$this->sending; }
    public function setsending($bool) { $old = $this->sending; if (MAIL_USE) { $this->sending = (bool)$bool; } return $old; }
     ...
     * ページ更新と同時にメールを送る。
     */
    public function update($obj, $arg)
    {
        if (!$this->sending) {
            return;
        }
